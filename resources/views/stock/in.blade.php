...
@section('content')
<h3 class="text-3xl font-bold text-gray-800 mb-6">Input Stok Masuk</h3>
<div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 max-w-2xl">
    <form action="{{ route('stock.storeIn') }}" method="POST">
        ...
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Supplier (Opsional)</label>
                <select name="supplier_id" class="mt-1 block w-full border rounded-md p-2">
            ...
            <div>
                ...
                <textarea name="notes" class="mt-1 block w-full border rounded-md p-2"></textarea>
            </div>
        </div>
        <div class="mt-6">
            <button type="submit" class="px-6 py-2 bg-green-600 text-white rounded-md">Simpan Stok Masuk</button>
        </div>
    </form>
</div>
@endsection